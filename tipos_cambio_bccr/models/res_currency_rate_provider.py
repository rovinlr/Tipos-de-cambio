...
class ResCurrencyRateProvider(models.Model):
    ...
    bccr_name = fields.Char(
        string='Nombre proveedor (legado)',
        default='Odoo',
        help='Campo legado del proveedor anterior del BCCR.',
    )
    bccr_email = fields.Char(
        string='Correo proveedor (legado)',
        default='noreply@example.com',
        help='Campo legado del proveedor anterior del BCCR.',
    )
    bccr_token = fields.Char(
        string='Token SDDE (legado)',
        help='Campo legado para respaldo con BCCR cuando Hacienda no responda.',
    )
    ...
    def _obtain_rates(self, base_currency, currencies, date_from, date_to):
        ...
        rates = {}
        for currency in currencies.filtered('active'):
            currency_name = currency.name
            if currency_name not in ('USD', 'EUR'):
                continue
            try:
                rates[currency] = self._hacienda_fetch_sale_rate(currency_name)
            except UserError:
